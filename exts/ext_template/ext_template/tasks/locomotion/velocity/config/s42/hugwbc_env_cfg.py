...
from ext_template.terrains import ROUGH_TERRAINS_CFG, RANDOM_TERRAINS_CFG
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        hugwbc_commands = ObsTerm(
            func=mdp.hugwbc_external_act_obs, params={"command_name": "hugwbc_cmd"}
        )
        joint_pos = ObsTerm(
...
class CurriculumCfg:
    ...
    external_act_noise_levels = CurrTerm(func=mdp.hugwbc_distribuate_levels)
...
class ActionsCfg:
    ...
    joint_pos = mdp.HugWBCActionCfg(asset_name="robot", joint_names=[".*"], scale=0.5, use_default_offset=True)


@configclass
class KuavoS42HugWBCEnvCfg(LocomotionVelocityRoughEnvCfg):
...
@configclass
class KuavoS42HugWBCEnvCfg_PLAY(KuavoS42HugWBCEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
...
@configclass
class KuavoS42FlatHugWBCEnvCfg(KuavoS42HugWBCEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
...
class KuavoS42FlatHugWBCEnvCfg_PLAY(KuavoS42FlatHugWBCEnvCfg):
