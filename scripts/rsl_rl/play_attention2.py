...
def visualize_attention(obs:TensorDict, attention:torch.Tensor, 
    ...
    # normalize attention shape to (B, L, W)
    if attention.dim() == 4:
        lastest_attention = attention[:, 0, ...]
    elif attention.dim() == 3:
        lastest_attention = attention
    else:
        return
    if lastest_attention.shape[-2:] != (L, W):
        if lastest_attention.numel() == B * L * W:
            lastest_attention = lastest_attention.reshape(B, L, W)
        else:
            return
    # sanitize invalid values
    lastest_scan_world = torch.nan_to_num(lastest_scan_world, nan=0.0, posinf=0.0, neginf=0.0)
    lastest_attention = torch.nan_to_num(lastest_attention, nan=0.0, posinf=0.0, neginf=0.0)
    # print(torch.sum(lastest_attention,dim=(1,2)))
    # 首先求取batch内最大的attention，然后归一化
    for env_idx in range(lastest_attention.size(0)):
        max_val = torch.max(lastest_attention[env_idx, :])
        if max_val > 0:
            lastest_attention[env_idx, :] = lastest_attention[env_idx, :] / (max_val + 1e-8)
    ...
    translations = lastest_scan_world.view(-1, 3).float()
    if translations.numel() == 0:
        return
    valid_mask = torch.isfinite(translations).all(dim=-1)
    if valid_mask.any():
        translations = translations[valid_mask]
        attention_indices = attention_indices[valid_mask]
    else:
        return
    markers.visualize(translations=translations.cpu(), marker_indices=attention_indices.cpu())
...
def main():
    ...
    print(obs.keys())
    print("output_attention:", ppo_runner.alg.policy.output_attention)
