...
# from leju_robot.rsl_rl_extensions.utils.on_policy_runner import RobotOnPolicyRunner
...
def main():
    ...
    export_policy_as_jit(
        ppo_runner.alg.policy, getattr(ppo_runner.alg.policy, "actor_obs_normalizer", None), path=export_model_dir, filename="policy.pt"
    )
    # NOTE: depending on rsl_rl API version, export_policy_as_onnx may require env.unwrapped as the first argument
    try:
        ...
        # Fall back to the other formulation
        export_policy_as_onnx(
            ppo_runner.alg.policy,
            normalizer=getattr(ppo_runner.alg.policy, "actor_obs_normalizer", None),
            path=export_model_dir,
